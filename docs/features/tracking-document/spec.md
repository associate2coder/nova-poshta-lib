...
- **Security review:** Required — the first module in the library that returns recipient/sender PII from a call that is not scoped to the caller's own counterparty (see AuthZ/AuthN impact above). Owner + gate: see §8.
...
- [ ] Does `getStatusDocuments` actually require a valid API key, or does Nova Poshta accept the call regardless (one source, `sirkostya009/go-novapost`, states "Доступність: Не вимагає використання API-ключа" — does not require use of an API key — unconfirmed by the other three sources, none of which contradict it either)? Default now: AC-09 is written to hold either way (the library sends the key like every other call and never independently validates it); §6.1's Security review stays Required until this is settled, since it changes whether this module's real-world authorization boundary is "any valid key" or "none at all." — owner: Tech Lead, due: before `sdd:implement tracking-document`
- [ ] Does the phone number affect which response fields Nova Poshta returns (e.g. masking recipient name/address when the supplied phone doesn't match its own record), and if so, does that show up as an empty string, `null`, or a genuinely absent field? Default now: every response field is typed as optional regardless of the answer (matching `common`'s AC-03 precedent), so the library's behavior doesn't depend on resolving this — but it should still be verified against a live API key before documenting the convenience method's phone parameter for consuming developers. — owner: Tech Lead, due: before `sdd:tasks tracking-document`
...
- [ ] Re-verify the 1-method TrackingDocument surface (§1) against Nova Poshta's live/official documentation once the portal is reachable by automated tooling — the 4-SDK cross-check is strong (all four agree field-for-field on the request and response shape) but is still a third-party source, the same caveat every shipped spec in this repo already carries. Default now: proceed on the SDK-verified single-method list. — owner: Tech Lead, due: before `sdd:implement tracking-document`
- [ ] Who performs the §6.1 security review this module requires, and what does it block (added during clarify)? Default now: the Tech Lead performs it, gating before `sdd:implement tracking-document` begins — same due-date pattern as the Tech-Lead-owned open questions above. — owner: Tech Lead, due: before `sdd:implement tracking-document`
