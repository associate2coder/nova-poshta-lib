...
| AC-06 — update clears an omitted backward-delivery instruction | `update` without a previously-set cash-on-delivery instruction clears it | unit (integration deferred — see note below) | The instruction is cleared, matching every other omitted field under the full-replace convention |
