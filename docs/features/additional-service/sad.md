...
| Read-return shape | List methods return `T[]` unmodified (`request()`); single-record methods return `T` (`requestFirst()`, throws on an empty array — ADR-0002); `checkReturnEditPossible` returns `{options: T[], info?: U}` (`requestEnvelope()` — ADR-0001; `info` optional since the envelope may omit it entirely) | additional-service §5 |
...
| ~~Re-verify the full 19-method surface...against Nova Poshta's live/official documentation once reachable~~ **Resolved 2026-09-23** across three capture rounds — round 1 quoted 9 shapes and fixed 4 real defects (`NonCash` boolean not string, `Pricing.FirstDayStorage` string not number, missing `RedirectOrderListItem.DocumentNumber`, missing `update` `OrderType`); round 3's exhaustive field-by-field audit found the rest of the module still had no genuine quote and a second capture closed it, plus fixed `OrderListFilters.Page`/`Limit`; round 5's independent re-verification found several of round 3's citations still pointed at quotes that didn't actually exist (`checkReturnPossible.Number`'s request, the three `save` responses, `createWaybillEdit`'s field list — the last sourced from a fabricated placeholder quote) and fixed all of them with genuine quotes, one via the exact docs URL found in `platx/go-nova-poshta`'s own source comments (the page isn't linked from the site's navigation). `ServiceType`'s full enum and `PaymentMethod`'s `"NonCash"` member are also now resolved (spec.md §8) — no open item remains. | Low | Closed | Tech Lead |
| ~~`OrderPricingEstimate.Pricing.Total`'s JSON shape genuinely disagrees across two official-docs examples~~ **Resolved 2026-09-23 (round 4)** — the claimed `"5.52"`-quoted-string example never actually existed anywhere in spec.md; both genuine calculate examples (return and redirect) show an unquoted JSON number. Typed `number` (was `number \| string`). | Low | Closed | Tech Lead |
