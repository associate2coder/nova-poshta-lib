...
|---|---|---|---|
| **Blocked design on this, then overridden (§1 ¶4):** does `checkReturnPossible`'s per-option `Ref` genuinely map onto `createReturn`'s `ReturnAddressRef`? A third-party bug report suggests `ReturnAddressRef` may expect a counterparty-address reference instead (spec.md §8, originally blocking before `design`) | High | §1's decision override lets `design` proceed since no §4–§9 decision here depends on the answer, and the assumption fails safely (a wrong mapping surfaces as `NovaPoshtaApiError`, never a corrupted order, per Flow 1) — but it genuinely blocks `/sdd:api` (the literal field must be pinned in the OpenAPI contract) and `/sdd:review` cannot PASS with it still open (`CLAUDE.md` sourcing policy) | Tech Lead — due before `sdd:api` |
| `checkReturnEditPossible`'s/`checkRedirectEditPossible`'s exact request field lists are unconfirmed (spec.md §8), as is whether `checkRedirectEditPossible` ever carries an `info` block the way `checkReturnEditPossible` does, and whether a malformed mixed request to either dual-purpose wire method (`Number` vs `Ref`/`OrderRef`+fields) could produce an ambiguous or wrong-shaped response neither this module's types nor its tests anticipate (spec.md §8 dispatch-rules OQ) — typed defensively (§4 decision 6) rather than with false precision | Medium | Loosely-typed request fields, re-verify against a live call before `sdd:ship`; provisional typing is documented inline in `src/types/additional-service.ts` | Tech Lead — due before integration tests run against a live key |
...
| A waybill-edit (ChangeEW) request has no typed `update` method — amending one means delete-then-recreate (spec.md §3, §8), a TOCTOU race no code change removes | Low | Accepted by design — documented in `spec.md` AC-19/§6.1; no mitigation beyond documentation, matching `scan-sheet`'s tolerated `addToTodaysScanSheet` TOCTOU precedent | Tech Lead |
| `orderTermExtension` (storage-term extension) excluded from scope — single, self-admittedly reverse-engineered source only (spec.md §1, §8) | Low | Tracked as an open question, not shipped as an AC; revisit once a 2nd agreeing source or official docs confirm it | Tech Lead |
| Re-verify the full 19-method surface — every request/response field, the `Ref`/`OrderRef` naming resolved during `clarify` (spec.md §1 note), and the ChangeEW `update` asymmetry — against Nova Poshta's live/official documentation once reachable by automated tooling (spec.md §8) | Low | Re-attempt during `sdd:review`/`sdd:ship`, same schedule every sibling module's equivalent OQ follows | Tech Lead |
