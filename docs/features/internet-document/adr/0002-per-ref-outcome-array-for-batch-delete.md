...
decision override, AC-08). `spec.md` §8 OQ-4 explicitly flags that whether Nova Poshta's live response
...
- `spec.md` §8 OQ-4's own stated default: build defensively now, since the live response shape is
  unconfirmed.
...
**Chosen:** Option 1. It is the only option that holds up under `spec.md` §8 OQ-4's own stated
...
- If `spec.md` §8 OQ-4 resolves to "the live API always returns a fully faithful per-Ref list," this
