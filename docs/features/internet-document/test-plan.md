...
|---|---|---|---|
| AC-01 — save happy path | `save` with valid Refs returns the created waybill's Ref and IntDocNumber | unit (integration deferred — see note below) | Waybill recorded; caller receives its `Ref`/`IntDocNumber` |
| AC-02 — discriminated payload compiles | `save`/`update` payload requires exactly the location fields its `ServiceType` leg needs (sender and recipient sides independently; `CargoType` is a plain field, not a structural axis — ADR-0004) | unit (type-level test) | Correct combination compiles; a payload mixing location fields from a different leg fails to compile |
| AC-03 — price calculator happy path | `getDocumentPrice` returns Nova Poshta's calculated price unchanged, with no link to a later `save` | unit (integration deferred — see note below) | Typed price returned; no recalculation, no stored linkage |
| AC-04 — delivery-date calculator happy path | `getDocumentDeliveryDate` returns Nova Poshta's calculated date unchanged | unit (integration deferred — see note below) | Typed date returned; same no-linkage behavior as AC-03 |
...
| AC-06 — update clears an omitted backward-delivery instruction | `update` without a previously-set cash-on-delivery instruction clears it; a *supplied* instruction passes through unmodified, never stripped | unit (integration deferred — see note below) | The instruction is cleared when omitted, matching every other omitted field under the full-replace convention; when supplied, it reaches `methodProperties` intact |
| AC-07 — batch delete happy path | `delete` with one or more Refs returns one outcome entry per submitted Ref, including a single-Ref call | unit (integration deferred — see note below) | Every submitted Ref appears in the result array, each marked removed |
...
| AC-10 — filtered list happy path | `getDocumentList` with a documented filter (e.g. date range) passes it through and returns the typed, unfiltered-equivalent shape | unit (integration deferred — see note below) | Exactly Nova Poshta's filtered response, no client-side re-filtering |
| AC-11 — printDocument happy path | `printDocument` for one or more Refs returns a single verified URL string, never through the JSON-envelope path | unit (integration deferred — see note below) | One URL string covering all requested waybills; construct-then-verify check passes |
| AC-12 — printMarkings happy path | `printMarkings` for one or more Refs returns a single verified URL string for the label | unit (integration deferred — see note below) | Same contract as AC-11, for the label |
...
- Missing required field for the chosen `ServiceType` leg on `save`/`update` → expected: compile-time failure (AC-02), never a runtime call.
...
- Seed strategy: no `data-model.md` schema exists for this feature (stateless library, no persistence) — test data is inline request/response fixtures per test file, one fixture set per `ServiceType` leg needed to exercise AC-02's discriminant, matching the fixture style already used in `test/unit/modules/counterparty.test.ts` and `test/unit/modules/address.test.ts`.
- Integration dependency: the real Nova Poshta API itself (no local throwaway dependency to seed — this library owns no datastore of its own); a future `save`-exercising integration test would create and then delete its own waybill(s) via the real `save`/`delete` calls so it leaves no live waybill behind under `NOVA_POSHTA_TEST_API_KEY`'s account.
- Cleanup boundary: per-test for any future write-path integration test — every `save` performed by a test is followed by a `delete` of the same Ref in that same test's teardown, regardless of assertion outcome, so a failed assertion never leaves a live waybill orphaned under the test account.
- **Integration-tier deferral (review 2026-09-21 finding 9):** `test/integration/internet-document.test.ts` currently covers only AC-09 (`getDocumentList`, which needs no seed Ref — matches `counterparty`'s `getCounterparties` precedent). AC-01/03/04/06/07/10/11/12's integration tier needs a real sender Ref, recipient Ref, and city/warehouse Refs to exercise honestly; this plan doesn't fabricate those, so their integration rows above read "unit (integration deferred)" rather than silently claiming coverage that doesn't exist. — owner: Tech Lead, due: once seed fixtures (a disposable sender/recipient counterparty + real warehouse Refs under the test account) exist for this suite to use.
