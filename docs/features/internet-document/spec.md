...
Traceability: module boundaries follow the same convention `address` and `counterparty` established (one folder per Nova Poshta model, dual ESM+CJS build, factory over the shared core client); the error contract reuses `NovaPoshtaApiError` unchanged; the full-replace `update` semantics and the return-`undefined`-on-empty-write behavior both reuse `address`'s ADR-0001, already scoped to extend to this module by name. AC-02's discriminated payload is built on `common`'s already-shipped `ServiceType` and `CargoType` reference types (`getServiceTypes`/`getCargoTypes`) rather than redefining those values here — only the discriminant *mechanism* tying them to the rest of a `save`/`update` payload is this module's own, per the dependency graph in `docs/roadmap.md`.
...
Decision override: a full-replace `update` call that omits a previously-set backward-delivery/afterpayment (cash-on-delivery) instruction clears it, exactly like every other omitted field — it is not carried forward. The failure-mode pass flagged this as a real risk (an address or phone correction that unintentionally clears the cash-to-collect instruction, delivering goods for free), but the fix is documentation and an explicit, tested AC (AC-06), not a partial-update mechanism this library's established full-replace convention doesn't otherwise use.
...
**Given** a consuming developer wants to create or update a waybill for a specific delivery method (warehouse-to-warehouse, warehouse-to-door, door-to-warehouse, or door-to-door) and cargo type
**When** they build the payload
**Then** the type system requires exactly the fields that delivery method and cargo type combination needs (for example, a warehouse Ref for a leg ending at a warehouse, a street/building/flat for a leg ending at a door) and rejects a payload mixing fields belonging to a different combination, at compile time — this checks the payload's own internal consistency only; it cannot verify that a warehouse or street Ref actually resolves to a real, matching location, which surfaces instead as Nova Poshta's own runtime decline (AC-14)
...
**Given** a consuming developer performs a waybill write (save, update, or delete)
...
**Given** a consuming developer wants to update an existing waybill that has a backward-delivery (cash-on-delivery) or afterpayment instruction on it
...
**Then** the system removes them and returns confirmation per AC-05's empty-on-success convention
...
**Then** the system denies the call by raising the standard error, passing through Nova Poshta's own message as-is, rather than performing or revealing the operation
...
**Then** the value returned is the authoritative, current one from Nova Poshta at the moment of the call — never a value invented or cached by this library — so any module that later accepts that same Ref/IntDocNumber can treat `internet-document`'s output as the single source for it
...
| Save/update discriminant guard | 100% of calls to `save`/`update` fail to compile if any field the chosen delivery-method/cargo-type combination requires is omitted, OR if the payload mixes fields belonging to a different combination (payload-internal consistency only, per AC-02) | static check in CI (type-level test) |
| Batch-delete result fidelity | 100% of batch `delete` calls surface a per-Ref outcome, never a single collapsed boolean, when Nova Poshta's response contains mixed results (AC-08) | unit test suite |
| Library-added overhead per call | Median total call time ≤ 5ms, measured with `fetch` stubbed to near-zero latency (no client-side caching, retries, or heavy parsing) | median across ≥30 repeated calls, benchmarked inside `test/unit/modules/internet-document` with `fetch` stubbed to near-zero latency (always runs in CI) |
...
- **Write-safety** — baseline: N/A (feature doesn't exist), target: 0 GitHub issues within 90 days of release reporting that an `update` call silently cleared a previously-set backward-delivery/afterpayment instruction, or that a batch `delete` call was misreported as fully successful when some Refs were actually rejected.

## 8. Open questions

- [ ] Re-verify the 8-method InternetDocument surface (§1), the full `save`/`update` field shape per delivery-method/cargo-type combination, AND whether the print-link methods' URL genuinely embeds the caller's API key (AC-13, §6.1) against Nova Poshta's live/official documentation once reachable — this spec's own drafting confirmed the portal still blocks automated fetches, and all three are currently inferred from community SDKs rather than the official source. Default now: proceed on the cross-checked list and the credential-embedding assumption, to be confirmed by live API calls. — owner: Tech Lead, due: before next release (`sdd:ship internet-document`)
- [ ] Should the shared core client be extended to expose Nova Poshta's pagination metadata (`totalCount`) and success-path warnings? `address` and `counterparty` both deferred this; carried forward unchanged here since `getDocumentList` is the same kind of growing, transactional list as `getCounterparties`. Default now: known limitation, not fixed in this feature (see AC-09). — owner: Tech Lead, due: before `sdd:design` of any module whose lookups depend on complete, multi-page results
- [ ] What type-level mechanism should `design` use to model the discriminated `ServiceType`/`CargoType` payload for `save`/`update` (AC-02), given that a naive reuse of `counterparty`'s own discriminated-union mechanism was built around a different axis (counterparty type, not delivery method × cargo type)? Default now: flagged for `design` to solve explicitly, scoped to this module's own types only. — owner: Tech Lead, due: before `sdd:design internet-document`
