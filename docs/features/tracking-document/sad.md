...
- `spec.md` §6.1: data classification confidential — a tracking response carries recipient/sender
  ...
  page. Security review completed 2026-09-22 by the Tech Lead during `sdd:review` (not performed in
  this design session; see `_review/review-2026-09-22.md`) — no findings requiring a code change
  (tracked in §11).
...
|---|---|---|---|
| Security review required before release — the first module in the library returning recipient/sender PII from a call with no per-counterparty ownership scoping at all (`spec.md` §6.1) | Resolved (2026-09-22) | Completed by the Tech Lead during `sdd:review`; no findings requiring a code change — see `_review/review-2026-09-22.md` | Tech Lead |
| Who performs the §6.1 security review this module requires, and what does it block? (`spec.md` §8 OQ) | Resolved (2026-09-22) | Performed by the Tech Lead during `sdd:review`, after implementation rather than before it — see `_review/review-2026-09-22.md` | Tech Lead |
| Does `getStatusDocuments` actually require a valid API key, or does Nova Poshta accept the call regardless? One cross-checked source states no key is required, unconfirmed by the other three (`spec.md` §8 OQ) | Resolved (2026-09-22) | Tech Lead's call during `sdd:review`: keep the current default and close the question — the library sends the key like every other call and never independently validates it either way, so this never changed the design, only the authorization-boundary claim in §2/§6.1 | Tech Lead |
| Does the phone number affect which response fields Nova Poshta returns (masking recipient detail on a non-matching phone), and if so how does that show up? (`spec.md` §8 OQ) | Open question | The shipped `TrackingStatus` types every field **required**, not optional as this row previously claimed (`sad.md`/`spec.md` corrected in place, 2026-09-22 — see `contracts/api-sync-report.md`'s "Open items"); if Nova Poshta masks a field, it would show up as an empty/zero value on that field, not an absent one. Still worth a live-API check before documenting `phone` further. | Tech Lead |
...
| Re-verify the 1-method TrackingDocument surface against Nova Poshta's live/official documentation once the portal is reachable by automated tooling — the 4-SDK cross-check agrees field-for-field but is still a third-party source, the same caveat every shipped spec in this repo carries (`spec.md` §8 OQ) | Resolved (2026-09-22) | Re-attempted during `sdd:review`: the legacy docs site is still 403; a newer official portal (`api-portal.novapost.com`) turned out to document a different, newer REST/JWT tracking API, not this module's legacy `modelName`/`calledMethod` surface. Proceeding on the SDK cross-check, strengthened to 3 independently-agreeing sources for the method surface and most fields (`contracts/api-sync-report.md`) | Tech Lead |
| Sizing risk: the response type's exhaustive field typing (`spec.md`'s "Resolved during clarify" note; corrected in place from an inaccurate 91-field estimate to the actual 118-field union) is materially larger than the estimate's original ~12-field PII subset, against this feature's current S estimate | Low (did not materialize) | `classify-size` was not re-run — implementation shipped within the original S/8-task estimate regardless of the wider field count, so this risk is closed without action | Tech Lead |
