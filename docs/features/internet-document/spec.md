...
> **Reference module / docs / channels used:** `docs/features/counterparty/spec.md` and `src/modules/counterparty/index.ts` / `src/types/counterparty.ts` (the full-replace `update` convention and the return-`undefined`-on-empty-write convention, both reused unchanged), `docs/features/address/spec.md` (the Ref-resolution precedent this module builds on), Nova Poshta's official developer portal (`developers.novaposhta.ua` — blocked every automated fetch attempted while drafting this spec, the same blocker every prior spec in this repo has hit), and — as of the original drafting — three independent community SDKs as the cross-check source for the in-scope method list and the `save`/`update` field shape: `platx/go-nova-poshta`'s `internetdocument` package, `maddsua/NovaPoshtaREST`, and `serj1chen/nova-poshta-sdk-php`'s `InternetDocument.php` class. **Widened (post-ship API-contract re-audit, 2026-09-22, per CLAUDE.md's API-contract sourcing policy):** two more independent sources re-fetched directly — `lis-dev/nova-poshta-api-2`'s `NovaPoshtaApi2.php` (a fourth PHP SDK citing Nova Poshta's own now-dead API 2.0 PDF, and the source of the print-link mechanism contradiction, see ADR-0003's latest amendment) and `shopanaio/carrier-api`'s `novaposhta-api-client` TypeScript package (an actively-maintained multi-carrier library, the source that corroborated `ServiceType`'s Postomat values and the sole dissent on `delete`'s batch capability, see ADR-0005). No ticket/Confluence/knowledge-base channel available.
...
### In-scope InternetDocument API methods (as of 2026-09-22)
...
1. `save` — create a new waybill; the required fields vary by delivery method (`ServiceType`: warehouse-to-warehouse, warehouse-to-door, door-to-warehouse, door-to-door — plus warehouse-to-postomat and door-to-postomat, confirmed as real values by a post-ship re-audit but not yet modeled as structural payload variants, see §8 OQ-1) and by cargo type (8 values — parcel, cargo, documents, pallet, tires/wheels, money, signed documents, trays) — see the discriminated-payload decision override below
2. `update` — full-replace an existing waybill's fields, matching `counterparty`'s identical convention
3. `delete` — remove exactly one waybill by Ref per call; a client-side `deleteBatch` convenience loops over multiple Refs (corrected post-ship from an originally-shipped single-call-batch design — see ADR-0005; a genuine server-side batch call is not confirmed by a majority of cross-checked sources)
...
Decision override: each `delete`/`deleteBatch` outcome is represented individually, not collapsed into one overall success/failure — Nova Poshta's own envelope can report a call successful while the Ref within it was rejected, and the failure-mode pass named the resulting silent half-success (a waybill the developer believes is cancelled but is still live and billable) as a distinct risk from the already-covered empty-on-success case (AC-05). **Corrected (post-ship API-contract re-audit, 2026-09-22, ADR-0005):** this was originally shipped as a single server call accepting multiple Refs; re-fetching all 4 cross-checked SDKs' actual source found 3 of 4 type the wire field as accepting exactly one Ref, with no source demonstrating a genuine multi-Ref call succeeding — `delete` now accepts exactly one Ref per call, and `deleteBatch` is this module's own sequential client-side loop over it, not a server-side batch capability.
...
**Given** a consuming developer holds the Ref of one of their own waybills
**When** they delete it
**Then** the system removes it and resolves one outcome confirming it was removed; a developer holding several Refs calls `deleteBatch`, which resolves one such outcome per submitted Ref, in submission order — see AC-08 for the case where a Ref isn't confirmed removed. **Corrected (post-ship API-contract re-audit, 2026-09-22, ADR-0005):** originally specified as a single batch-capable call accepting multiple Refs; 3 of 4 re-fetched cross-checked sources type Nova Poshta's `delete` as accepting exactly one Ref per call, with no source demonstrating a genuine multi-Ref call succeeding, so `deleteBatch` is now this module's own sequential client-side loop over single-Ref `delete` calls, not a single server call

### AC-08 (US-07) — error

**Given** a consuming developer deletes a waybill Ref, alone or as part of a `deleteBatch` call
**When** Nova Poshta's response reports the call successful but does not confirm that Ref removed
**Then** the system represents that outcome as `Removed: false` with Nova Poshta's own reason when one is given, as part of the same outcome shape every `delete`/`deleteBatch` call returns (AC-07), never throwing for that Ref alone (a full network or malformed-response failure still raises the standard error per AC-14/AC-16); a `deleteBatch` call surfaces this independently per Ref, since each Ref is now its own underlying `delete` call rather than one shared response
...
### AC-11 (US-09) — happy path — **PROVISIONAL, see below**
...
**Then** the system returns a single URL string covering all requested waybills — Nova Poshta issues one combined printable document per call, not one link per Ref (confirmed against Nova Poshta's own `devcenter.novaposhta.ua` documentation, see §8 OQ-1) — without routing the request through this library's shared JSON-envelope handling (see §1 decision override) — a failure to obtain the link (an invalid Ref, a document Nova Poshta hasn't finished materializing) raises the standard error rather than returning a link that resolves to a blank or error page. **PROVISIONAL (post-ship API-contract re-audit, 2026-09-22):** the exact URL-construction mechanism is contested across sources — a fourth cross-checked SDK builds it differently (comma-joined single segment vs. this module's repeated segments) and its own live-hitting tests prove these methods are *also* reachable as plain enveloped calls, a path never attempted here. This AC is downgraded from confirmed happy-path to provisional pending resolution — see ADR-0003's latest amendment and §8 OQ-1.

### AC-12 (US-09) — happy path — **PROVISIONAL, see AC-11**

**Given** a consuming developer holds the Ref(s) of one or more existing waybills
**When** they request a print-ready link for the shipping label
**Then** the system returns it with the same contract as AC-11 (a single URL string covering all requested labels), for the label rather than the waybill document. Carries the same AC-11 provisional caveat.
...
- **Known consideration (confirmed 2026-09-22):** the print-link methods' returned URL embeds the caller's API key directly — per the community SDKs cross-checked in §1, and now also confirmed directly from Nova Poshta's own `devcenter.novaposhta.ua` documentation (see §8 OQ-1's seventh-pass narrowing; not yet re-verified against a live API response, since this remains a documentation source rather than a call this spec's own drafting could make). Any holder of a shared print link can act with the caller's full account privileges, not merely view a document — this is documented in AC-13.
- **Abuse cases:**
...
- [ ] Re-verify the 8-method InternetDocument surface (§1), the full `save`/`update` field shape per delivery-method/cargo-type combination, whether the print-link methods' URL genuinely embeds the caller's API key (AC-13, §6.1), whether the print methods genuinely return one combined link per call rather than one per Ref (AC-11, AC-12), what format/parameter options (PDF vs HTML, copies, label size) the print methods' typed request should expose, AND how a caller can tell a print request failed given the print path does not read a JSON envelope (AC-11, AC-12, §1 decision override) — all against Nova Poshta's live/official documentation once reachable. This spec's own drafting confirmed the portal still blocks automated fetches, and all of the above are currently inferred from community SDKs rather than the official source. Default now: proceed on the cross-checked list, the credential-embedding assumption, and the single-combined-link assumption, to be confirmed by live API calls. **Sharpened (review remediation, third pass, 2026-09-21):** the shipped verification (`GET` + discard body, ADR-0003 amendment) can only detect a non-2xx status — it cannot detect a blank/error HTML page returned with a `200` status, which is exactly the save-then-print-race risk AC-11 was written to guard against. Re-verifying against the live docs should specifically determine whether Nova Poshta's print endpoint signals that failure mode some other way (a response header, a body marker) this library could check without holding the body open. **Narrowed (review remediation, seventh pass, 2026-09-22):** re-attempted the official portal named above — `developers.novaposhta.ua` still returns HTTP 403 to every automated fetch, unchanged since this spec's original drafting. A *different* official Nova Poshta domain, `devcenter.novaposhta.ua` (linked from Nova Poshta's own `nova-poshta-api-2`/`riddman/nova-poshta-api` community SDKs as "API 2.0's" home), is live but its TLS handshake fails for a direct fetch; a search engine's cached crawl of its own developer blog post was reachable, and **confirms two of this open question's items directly from Nova Poshta's own published URL pattern** — `https://my.novaposhta.ua/orders/printDocument/orders[]/[reference or number]/type/[html or pdf]/apiKey/[YOUR_KEY]` (and the equivalent for `printMarkings`): (1) the URL genuinely embeds the caller's `apiKey` as a literal path segment, confirming AC-13/§6.1's credential-exposure assumption as designed, not merely inferred; (2) multiple `orders[]` segments are accepted in one URL ("multiple TTNs can be printed in a single request"), confirming the single-combined-link assumption behind AC-11/AC-12 rather than one link per Ref — both match the structural shape of this module's shipped `buildAndVerifyPrintLink` implementation (`src/modules/internet-document/index.ts`: repeated `orders[]/<ref>` segments + trailing `apiKey/<key>`). **This confirmation does not extend to the `type` value's exact casing or to the existence/shape of a `copies` parameter** — the quoted pattern's `type/[html or pdf]` is a search-summary paraphrase, not a verbatim quote, and no `copies` segment appears in it at all; those two remain SDK-sourced only (see the eighth-pass correction below). Still unconfirmed against a primary source (no reachable page gave the full field/parameter list): the exact `save`/`update` required-field matrix per `ServiceType` (this OQ's own field-shape question, and OQ-3 below), `delete`'s per-Ref response shape (OQ-4 below), pagination `totalCount` (OQ-2 below), and the print endpoint's 200-status blank-page failure signal (the sharpened item just above) — these remain open on the same "proceed on the cross-checked default, confirm before/at next re-verification" basis. (Also noted in passing, out of scope for this feature: Nova Poshta/NovaPost now also operates a separate, newer REST API at `api-portal.novapost.com` — JWT-authenticated, URL-versioned under `/v1.0/`, method names and shapes unrelated to the `apiKey`/`modelName`/`calledMethod`/`methodProperties` v2.0 JSON-RPC API this library wraps; not a source for this module's behavior, but worth a future roadmap note if Nova Poshta ever deprecates v2.0 in its favor.) **Corrected (review remediation, eighth pass, 2026-09-22):** the seventh pass's own confidence-raising exercise (re-fetching the PHP SDK to check the `type`-value casing claim above) surfaced a real bug it then fixed: `Copies`'s cross-checked *existence* on the PHP SDK's request struct had been read as "it's a URL segment," but re-fetching that SDK's actual `getPrintLink()` method body shows `Copies` is never its own segment — `"fourfold"` repeats each Ref's segment twice, every other value (including `"double"`) repeats it once, and the previously-shipped `/copies/<value>` segment never corresponded to anything Nova Poshta defines. Fixed in the same pass (`ADR-0003`'s amendment log). The `type` value's exact casing (`"Pdf"`/`"Html"`, re-confirmed directly from the same SDK source's constant *values*, not just their names) is left as shipped — the PHP source is unambiguous there, only the earlier search-summary paraphrase was ambiguous. **Sharpened again (post-ship API-contract re-audit, 2026-09-22, per CLAUDE.md's API-contract sourcing policy):** widening the source pool to 5 total surfaced a genuine mechanism-level contradiction, not just a detail-level one. `lis-dev/nova-poshta-api-2`'s `printGetLink()` builds the URL with all Refs **comma-joined into one** `orders[]/` segment, lowercase `type`, and **no** `Copies` segment — versus this module's (and `serj1chen`'s) repeated-segment, capitalized-`Type` construction. Worse: `lis-dev`'s own live-hitting test suite (`NovaPoshtaApi2Test.php::testPrintDocument`) proves `printDocument`/`printMarkings` are reachable as **plain enveloped `calledMethod` requests** (a bogus Ref returns real Nova Poshta error code `20000300415` through the normal JSON cycle) — a path this module's entire `ADR-0003` premise assumed was impossible for these two methods. Neither SDK author verified their own URL-shortcut against a live *resolved* link. **This is now the single most urgent unresolved item in this spec** — AC-11/AC-12/AC-13 are downgraded to provisional pending resolution (see ADR-0003's amendment log). Default now: ship the existing live-verified construction unchanged (it is at least confirmed to resolve to *something* with a 2xx status), continue documenting it as provisional, and prioritize a live API key check or a reachable primary source over any further SDK cross-referencing, which has now demonstrably reached diminishing, contradictory returns. — owner: Tech Lead, due: before next release touching print-link behavior
...
- [x] What type-level mechanism should `design` use to model the discriminated `ServiceType`/`CargoType` payload for `save`/`update` (AC-02), given that a naive reuse of `counterparty`'s own discriminated-union mechanism was built around a different axis (counterparty type, not delivery method × cargo type)? This also covers the still-undocumented question of which exact fields each of the up to 16 delivery-method × cargo-type combinations actually requires — not just how the compile-time check is implemented, but what it checks against. Default now: flagged for `design` to solve explicitly, scoped to this module's own types only, cross-checked against the same three community SDKs used elsewhere in this spec pending official docs. **Resolved for cardinality (post-ship API-contract re-audit, 2026-09-22):** the standalone `ServiceType`/`CargoType` types are widened to their full cross-checked cardinality (6 and 8 respectively — was 4 and 4), confirmed by 2 independent sources for `ServiceType`'s Postomat pair. `save`/`update`'s discriminated union still models only the original 4 `ServiceType` values as structural variants — the Postomat pair's required-field shape (a seat/dimensions block) has only 1 confirming source (`shopanaio/carrier-api`), not the 2 this repo's sourcing policy requires, so it's deliberately not invented here. New open item: confirm the Postomat leg's field shape against a second source before adding `WarehousePostomat`/`DoorsPostomat` as `save`/`update` structural variants. — owner: Tech Lead, due: before Postomat delivery is added as a `save`/`update` variant
...
- [x] Confirm whether `delete`'s per-Ref outcome (AC-08) is actually distinguishable in Nova Poshta's live response shape, or whether the "mixed batch result" risk the failure-mode pass identified is purely theoretical for this endpoint. Default now: build AC-08's per-Ref representation defensively; downgrade to a simpler contract if the live API never actually returns a mixed result. **Resolved (post-ship API-contract re-audit, 2026-09-22, ADR-0005):** downgraded exactly as this OQ's own default anticipated — re-fetching all 4 cross-checked sources found 3 of 4 type `delete`'s wire field as accepting exactly one Ref, with no source demonstrating a genuine multi-Ref call succeeding. `delete` now accepts one Ref per call; `deleteBatch` is this module's own sequential client-side loop, not a server-side batch capability. See AC-07/AC-08, ADR-0005.
